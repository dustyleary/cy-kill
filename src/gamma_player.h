    ...
    void assertGoodState(Board& b) {
        ...
        FOREACH_NAT(Point, p, {
            if(black.weights[p] != 0) {
                if(!b.isValidMove(BoardState::BLACK(), p) || b.isSimpleEye(BoardState::BLACK(), p)) {
                    b.dump();
                    fprintf(stderr, "invalid move: %s has a non-zero weight\n", p.toGtpVertex(b.getSize()).c_str());
                    Pat3 pat = b.getPatternAt(p);
                    fprintf(stderr, "pat:\n");
                    pat.dump();
                    ASSERT(b.isValidMove(BoardState::BLACK(), p));
                }
            }
            if(white.weights[p] != 0) {
                if(!b.isValidMove(BoardState::WHITE(), p) || b.isSimpleEye(BoardState::WHITE(), p)) {
                    b.dump();
                    fprintf(stderr, "invalid move: %s has a non-zero weight\n", p.toGtpVertex(b.getSize()).c_str());
                    Pat3 pat = b.getPatternAt(p);
                    fprintf(stderr, "pat:\n");
                    pat.dump();
                    ASSERT(b.isValidMove(BoardState::WHITE(), p));
                }
            }
        });
            ...
            for(uint y=0; y<b.getSize(); y++) {
                ...
                if(b.isValidMove(BoardState::BLACK(), p) && !b.isSimpleEye(BoardState::BLACK(), p)) {
                    if(black.weights[p] == 0.0) {
                        ...
                        ASSERT(black.weights[p] != 0.0);
                    }
                } else {
                    ASSERT(black.weights[p] == 0.0);
                }
                if(b.isValidMove(BoardState::WHITE(), p) && !b.isSimpleEye(BoardState::WHITE(), p)) {
                    if(white.weights[p] == 0.0) {
                        b.dump();
                        fprintf(stderr, "expect weight non-zero: %s\n", p.toGtpVertex(b.getSize()).c_str());
                        Pat3 pat = b.getPatternAt(p);
                        fprintf(stderr, "pat:\n");
